...
<script lang="ts" setup>
  ...
  import { getFileList, deleteFile } from '@/api/pms/file';
  ...
  async function handleDelete(record: Recordable) {
    const result = await deleteFile({ ids: [record.id] });
    if (result.code === 0) {
      await reload();
    }
  }
  ...
  async function handleBatchDelete() {
    Modal.confirm({
      ...
      async onOk() {
        const result = await deleteFile({ ids: selectedIds.value as string[] });
        if (result.code === 0) {
